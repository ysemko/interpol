...
AppDataSource.initialize().then(async () => {
    ...
    try {
        ...
        const countries: Country[] = await AppDataSource.manager
            ...
            .distinctOn(["country.country_id"]).orderBy("country.country_id", "DESC")
            .getMany();
        ...
        for (const country of countries) {
            ...
            for (let age = 0; age < 100; age += 10) {
                //Males
                await fetchRedNotices(stats, { arrestWarrantCountryId: country.country_id, ageMax: age + 10, ageMin: age, sexId: "M" });
                await fetchRedNotices(stats, { nationality: country.country_id, ageMax: age + 10, ageMin: age, sexId: "M" });
                await fetchYellowNotices(stats, { nationality: country.country_id, ageMax: age + 10, ageMin: age, sexId: "M" });
                //Females
                await fetchRedNotices(stats, { arrestWarrantCountryId: country.country_id, ageMax: age + 10, ageMin: age, sexId: "F" });
                await fetchRedNotices(stats, { nationality: country.country_id, ageMax: age + 10, ageMin: age, sexId: "F" });
                await fetchYellowNotices(stats, { nationality: country.country_id, ageMax: age + 10, ageMin: age, sexId: "F" });
                //Unknown
                await fetchRedNotices(stats, { arrestWarrantCountryId: country.country_id, ageMax: age + 10, ageMin: age, sexId: "U" });
                await fetchRedNotices(stats, { nationality: country.country_id, ageMax: age + 10, ageMin: age, sexId: "U" });
                await fetchYellowNotices(stats, { nationality: country.country_id, ageMax: age + 10, ageMin: age, sexId: "U" });
                // NULL
                await fetchRedNotices(stats, { arrestWarrantCountryId: country.country_id, ageMax: age + 10, ageMin: age });
                await fetchRedNotices(stats, { nationality: country.country_id, ageMax: age + 10, ageMin: age });
                await fetchYellowNotices(stats, { nationality: country.country_id, ageMax: age + 10, ageMin: age });
            }
...
function backgroundLogs(stats: any, start) {
    loggingIntervalId = setInterval(() => {
        ...
        console.log("Estimated time left: " + ((Date.now() - start) / stats.activeCountry) * (stats.countCountries - stats.activeCountry) / 1000 + " seconds");
        console.log("----------------------------------------");
    }, 5000);
}
